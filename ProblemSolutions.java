...
class ProblemSolutions {
    ...
    public boolean canFinish(int numExams, int[][] prerequisites) {
        ...
        // Fill adjacency list and compute in-degrees
        ...
        // return true; otherwise, there's a cycle
    ...
    private void dfs(int curr, boolean[] visited, int[][] adjMatrix) {
        visited[curr] = true;

        for (int neighbor = 0; neighbor < adjMatrix.length; neighbor++) {

            if ((adjMatrix[curr][neighbor] == 1 || adjMatrix[neighbor][curr] == 1)
                    && !visited[neighbor]) {
